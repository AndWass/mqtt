...
struct handshake_op {
    ...
    template<class Self>
    void operator()(Self &self, boost::system::error_code ec = {}, mqtt::fixed_header header = {}) {
            ...
            if (!ec.failed()) {
                ...
                if (!ec) {
                    if (header.first_byte != 0x20 || header.remaining_length != 2) {
                        ec = mqtt::make_error_code(error::invalid_connect_response);
                    } else if (buffer_[1] == 1) {
                        ec = mqtt::make_error_code(error::unacceptable_protocol_version);
                    } else if (buffer_[1] == 2) {
                        ec = mqtt::make_error_code(error::identifier_rejected);
                    } else if (buffer_[1] == 3) {
                        ec = mqtt::make_error_code(error::server_unavailable);
                    } else if (buffer_[1] == 4) {
                        ec = mqtt::make_error_code(error::bad_username_or_password);
                    } else if (buffer_[1] == 5) {
                        ec = mqtt::make_error_code(error::unauthorized);
                    }
                }
            }
