...
TEST(Stream, AsyncWriteShort) {
    ...
    size_t amount_written = 0;
    bool called = false;
    stream.async_write_short(0x10, {0x30, 0x31, 0x33}, 3, [&](boost::system::error_code ec, size_t n) {
        final_ec = ec;
        amount_written = n;
        called = true;
    });
    ...
    EXPECT_TRUE(called);
    EXPECT_FALSE(final_ec.failed());
    EXPECT_EQ(amount_written, 5);
}

TEST(Stream, AsyncWriteBadShort) {
    namespace net = boost::asio;
    net::io_context io;
    mqtt::stream<boost::beast::test::stream> stream(io);
    ...
    size_t amount_written = 0;
    bool called = false;
    EXPECT_THROW(stream.async_write_short(0x10, {0x30, 0x31, 0x33}, 4, [&](boost::system::error_code ec, size_t n) {
        final_ec = ec;
        amount_written = n;
        called = true;
    }),
